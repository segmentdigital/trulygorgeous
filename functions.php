...
genesis_register_sidebar( array(
	'id'				=> 'imageslider',
	'name'			=> __( 'Image Slider', 'imageslider' ),
	'description'	=> __( 'Place the Genesis Responsive Slider Here', 'imageslider' ),
) );

add_action( 'genesis_after_header', 'imageslider' );
function imageslider() {

	if ( is_active_sidebar( 'imageslider' ) ) {
		if(!is_page('wedding-dresses') && !is_page('contact') && !is_page('brides-gallery-testimonials')){
			echo '<div id="featured-image"><div class="imageslider">';	
			dynamic_sidebar( 'imageslider' );
			echo '</div></div>';
		}
	}
}
